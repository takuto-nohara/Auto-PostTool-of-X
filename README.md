...
```javascript
createTrigger(); // 投稿時刻に自動実行されるトリガーを作成 (TriggerManager.gs)
```

**動作:**
- 現在時刻が投稿指定時刻を過ぎている場合 → 翌日の指定時刻にトリガー設定
- 現在時刻が投稿指定時刻の5分前より前の場合 → 当日の指定時刻にトリガー設定
- 現在時刻が5分前～指定時刻の間の場合 → 翌日の指定時刻にトリガー設定（安全のため）

**判定ロジック:**
1. 投稿時刻を1秒でも過ぎていれば → 即座に翌日設定可能
2. 5分以上余裕があれば → 当日設定
3. 5分以内の場合 → 安全のため翌日設定

例（投稿時刻が19:29の場合）:
- 15:00に`createTrigger()`実行 → 当日19:29に実行（4時間29分後）
- 19:20に`createTrigger()`実行 → 当日19:29に実行（9分後）
- 19:25に`createTrigger()`実行 → 翌日19:29に実行（24時間4分後、5分以内のため安全策）
- 19:29:01に`createTrigger()`実行 → 翌日19:29に実行（23時間59分後、1秒過ぎた時点で翌日）
- 20:00に`createTrigger()`実行 → 翌日19:29に実行（23時間29分後）
